...
while True:
    ...
    def mostrar_resultados(tabela_df):
        if nome.lower() == 'todos':
            ...
            if intervalo == 's':
                ...
                intervalo_df = tabela_df.loc[(tabela_df['Criado em']>= data_inicial) & (tabela_df['Criado em'] <= data_final)]
                curtidas_recebidas_totalf(intervalo_df)
                curtidas_enviadas_totalf(intervalo_df)
            else:
                intervalo_df = tabela_df
                curtidas_recebidas_totalf(tabela_df)
                curtidas_enviadas_totalf(tabela_df)
        else:
            ...
            # O código não está ledno essa parte 
            if intervalo == 's':    
                data_inicial = input('Insira a data inicial:')
                data_final = input('Insira a data final:')
                intervalo_df = tabela_df.loc[(tabela_df['Criado em']>= data_inicial) & (tabela_df['Criado em'] <= data_final)]
                tabela_df = intervalo_df
                ...
                if escolha == 1:
                    curtidas_enviadasf(tabela_df)
                elif escolha == 2:
                    curtidas_recebidasf(tabela_df)
                elif escolha == 3:
                    total_de_curtidasf(tabela_df)
                elif escolha == 4:
                    total_de_xpf(tabela_df)
